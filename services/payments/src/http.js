...
// Opaque ids. Consumers must never parse one -- contracts/payments.openapi.yaml
// says so, and that is what lets us change the scheme later.
...
export function requireOptionalTimestamp(value, field) {
...
export function requireUrl(value, field) {
  ...
  try {
    ...
    throw new InvalidInput(`${field} must be a valid URL`);
  }
}
